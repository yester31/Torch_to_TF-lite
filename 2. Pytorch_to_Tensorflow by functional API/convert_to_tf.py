...
print('resnet18 max index (tensorflow): {} , value : {}, class name : {}'.format(max_index2, max_value2, class_name[max_index2]))


resnet_torch.eval()
torch_output = resnet_torch(img_torch) # warm up
dur_time2 = 0
for i in range(iteration):
    begin = time.time()
...
print('resnet18 max index (pytorch): {} , value : {}, class name : {}'.format(max_index, max_value, class_name[max_index]))


# Find max difference between model outputs
diff = np.abs(tf_output - torch_output.detach().numpy())
